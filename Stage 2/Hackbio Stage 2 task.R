...
fviz_eig(a, addlabels = TRUE, geom = c("bar", "line"), barfill = "green",
         barcolor = "black", linecolor = "red", ylim = c(0, 70))
...
fviz_pca_var(a, col.var = "cos2",
             gradient.cols = c("#004181", "#0077BA", "#4E564D", "#6BA3D7"),
             repel = TRUE)
...
b <- PCA(t(dataset[,-c(1:2)]), scale.unit = TRUE, graph = FALSE)


# For visualization, the fviz_pca_ind function is then used
c <- fviz_pca_ind(b, col.ind = "cos2", 
             gradient.cols = c("#FF0000", "#CCFF00", "#00FF66", "#0066FF"), 
             repel = TRUE)
